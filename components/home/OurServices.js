      ...
      <Col xs={12} md={4} className="p-3" key={`service-wrapper-${index}`}>
        <StyledCard key={`service-card-${index}`}>
          ...
          <Card.Body key={`service-card-body-${index}`}>
            <Card.Title key={`service-card-title-${index}`}>
              {translate(`home:services-list.${index}.title`)}
            </Card.Title>
            {translate(`home:services-list.${index}.subtitle`, {
              returnObjects: true,
            }).map((element, i) => (
  ...
  return (
    <>
      <Container>
        ...
        <Row className="text-center p-2">{servicesCards}</Row>
      </Container>
    </>
  );
};
